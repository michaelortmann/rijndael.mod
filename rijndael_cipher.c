...
static word  rcon_tab[RC_LENGTH];   // table of round constants
...
cf_dec c_name(set_blk)(const word n_bytes, c_name(aes) *cx)
{
	...
	{     
        return (n_bytes ? cx->mode &= ~0x08, aes_bad : (aes_ret)(nc << 2));
    }

    cx->mode = cx->mode & ~0x0c | 0x0c;
    nc = n_bytes >> 2;
    return aes_good;
}
...
{   word    *kf, *kt, rci;
    ...
    if((n_bytes != 16 && n_bytes != 24 && n_bytes != 32) || (!(f & 1) && !(f & 2)))
        return (n_bytes ? cx->mode &= ~0x0c, aes_bad : (aes_ret)(cx->Nkey << 2));
